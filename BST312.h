...
void BST_312 <ItemType>::insertItem(TreeNode*& t, const ItemType& newItem)
{
    ...
    //YOUR CODE GOES HERE
    if(t!=NULL)//can't insert items if the node being searched for isn't null
    {
        if(t->data > newItem)
    ...
    else if (t==NULL)//create a temporary TreeNode and set all values before setting the parameter 't' equal to the temporary TreeNode
    {
...
{
    //YOUR CODE GOES HERE
    int nodeCount=1;//default value to add to a count of nodes, regardless if nodes left or right are NULL or not 

    if(t->left!=NULL)
        nodeCount+=countNodes(t->left);
    if(t->right!=NULL)
        nodeCount+=countNodes(t->right);
...
{
    //YOUR CODE GOES HERE
    vector<ItemType> test;//temporary vector to store the result of a preOrderTraversal starting at root
    preOrderTraversal(root,test);
    return test;
...
{
    //YOUR CODE GOES HERE
    vector<ItemType> test;//temporary vector to store the result of a inOrderTraversal starting at root
...
bool BST_312 <ItemType>::isItemInTree(const ItemType& item)
{
  ...
  //YOUR CODE GOES HERE
    ...
    {
        if(item==test->data)
            return true;//item is in tree if the item being searched for is the same as the data of the tempoary tree node
        
        else if(item<test->data)
            test=test->left;
